             ...
             {[PaymentMethod.CASH, PaymentMethod.PIX, PaymentMethod.ACCOUNT, PaymentMethod.DEBIT, PaymentMethod.CREDIT].map(m => {
               ...
               return (
                 <button 
                  ...
                  className={`py-2.5 rounded-xl text-[7.5px] font-black italic uppercase shadow-sm transition-all border ${isSelected ? `${activeClass} scale-105 shadow-md` : 'bg-white text-slate-400 border-slate-100 hover:bg-slate-50'}`}
                 >
                   {m}
                 </button>
               );
