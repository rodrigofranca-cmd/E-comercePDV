...
export const FinanceTab: React.FC<{ state: any }> = ({ state }) => {
  ...
  const stats = useMemo(() => {
    ...
    // Filtro de data simplificado (para uma aplicação real, compararíamos timestamps)
    state.orders.forEach((o: any) => {
      // REGRA: Somente vendas CONFIRMADAS são computadas no financeiro.
      // Pedidos PENDENTES e CANCELADOS são ignorados para precisão de caixa.
      ...
      // Cálculo Real do Lucro: Diferença entre Preço de Venda e Preço de Compra
      o.items.forEach((item: any) => {
        const product = state.products.find((p: any) => p.id === item.productId);
        if (product) {
          // Se o produto existe, usamos o preço de compra cadastrado
          totalProfit += (item.price - product.purchasePrice) * item.quantity;
        } else {
          // Fallback caso o produto tenha sido removido: estima-se 30% de margem
       ...
       <div className="flex gap-2 overflow-x-auto carousel-scroll py-2 px-1">
         {['TUDO', 'DIARIO', 'SEMANAL', 'MENSAL', 'ANUAL'].map(f => (
           <button 
            ...
            className={`px-5 py-2 rounded-full text-[8px] font-black italic uppercase transition-all border-2 ${
                filter === f 
